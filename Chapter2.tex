...
(a)There are 36 possible events：
...
(b)Prove$P(A\bigcap B)=P(A)P(B)$


\noindent\textbf{2.10}

(a)Empty sets and complete sets are independent of any event：

$$P(A\bigcap\Omega)=P(A)=1*P(A)=P(\Omega)*P(A)$$
$$P(A\bigcap\phi)=P(\phi)=0=P(\phi)*P(A)$$

(b)Prove when$P(A)=0$or1，A is independent of any event：for any$B\in\Omega$
...
(c)$P(A^c \bigcap A) = P(A)P(A^c)$

we have $0=P(A)(1-P(A))\Rightarrow P(A)\in\{0,1\}$

(d)$P(A \bigcap A) = P(A)P(A)$,则$P(A)=0,1$

(e)$\Omega=(1,1),(1,0),(0,1),(0,0)$
...
(g)Necessity ：$\frac{|A\bigcap B|}{n}=P(A \bigcap B)=P(A)P(B)=\frac{|A|}{n}\frac{|B|}{n}$
...
(h)$|A\bigcap B|*n = |A||B|$,$\Rightarrow n\leq |A|\leq n \Rightarrow A\neq\phi or \Omega$

(j)Let's take a counter example: roll a die and set the A event to$\{1,2,3\}$,B event is set to$\{1,2,4\}$,C event is set to$\{1,4,5,6\}$
...
(d)$A_i$  pairwise i $\Leftrightarrow$ $I\{ A_i \}$  pairwise i
...
(a)For any $A\in B(R)\Rightarrow $A is open,

so,$f^{-1}(A)$is open，so $f^{-1}(A)\in B(R)$

(b)X is known to be a random variable ，$f(x)=|x|$ continuous。
